...
var StationsPageModule = /** @class */ (function () {
    function StationsPageModule() {
    }
...
var StationsPage = /** @class */ (function () {
    function StationsPage(commonService, ajaxService, activatedRoute, location) {
        var _this = this;
        ...
        this.getPlaceDetails = function (type) {
            ...
            //   });
            if (type === "Fuel") {
                _this.searchType = "gas_station";
            }
            else if (type === "Vehicle Repair") {
                _this.searchType = "car_repair";
            }
            else if (type === "Police Station") {
                _this.searchType = "police";
            }
            var googleUrl = src_environments_environment__WEBPACK_IMPORTED_MODULE_5__["serverUrl"].web + '/login/nearby/' + _this.lat + '/' + _this.lng + '/' + _this.searchType;
            _this.commonService.presentLoader();
            _this.ajaxService.ajaxGet(googleUrl).subscribe(function (res) {
                // console.log(res)
                _this.commonService.dismissLoader();
                _this.setResultantPlaceDetails(res);
            });
        };
        this.setResultantPlaceDetails = function (res) {
            //console.log(res);
            _this.resultantNearByPlaceDetails = res;
            var resultedArray = [];
            if (res.status == 'OVER_QUERY_LIMIT' || res.status == "REQUEST_DENIED") {
                _this.commonService.presentAlert('Alert', 'Something went wrong');
                _this.headerText = _this.nearbyType;
                _this.key[_this.nearbyType][0]['results'] = null;
                _this.key[_this.nearbyType][0]['status'] = null;
                _this.key[_this.nearbyType][0]['updater'] = false;
                localStorage.setItem(_this.paramVin, JSON.stringify(_this.key));
            }
            else {
                ...
                Object.assign(_this.key[_this.nearbyType][0], response);
                _this.headerText = _this.nearbyType;
                _this.key[_this.nearbyType][0]['updater'] = true;
                localStorage.setItem(_this.paramVin, JSON.stringify(_this.key));
            }
            _this.resultantNearByPlaceDetails = res.results;
            _this.commonService.dismissLoader();
        };
        ...
        this.contactSelected = function (placeId) {
            ...
            var url = src_environments_environment__WEBPACK_IMPORTED_MODULE_5__["serverUrl"].web + '/login/placeid/' + placeId;
            this.ajaxService.ajaxGet(url)
                .subscribe(function (res) {
                _this.commonService.dismissLoader();
                var phoneNumber = res.result.formatted_phone_number;
                if (res.status == 'OVER_QUERY_LIMIT' || phoneNumber == 'OVER_QUERY_LIMIT') {
                    _this.commonService.presentAlert('Alert', 'Something went wrong');
                }
                if (phoneNumber === undefined) {
                    _this.commonService.presentAlert('Alert', 'Sorry,Contact is Not Available');
                }
                else {
    ...
    StationsPage.prototype.locationBack = function () {
        this.location.back();
    };
    StationsPage.prototype.ngOnInit = function () {
        var selectedVin = JSON.parse(localStorage.getItem('selectedVin'));
    ...
    StationsPage.ctorParameters = function () { return [
...
//# sourceMappingURL=nearby-stations-stations-module-es5.js.map