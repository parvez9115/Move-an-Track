...
@Component({
  ...
  templateUrl: "./device-extend-oneyear-invoice-details.page.html",
  styleUrls: ["./device-extend-oneyear-invoice-details.page.scss"],
})
export class DeviceExtendOneyearInvoiceDetailsPage implements OnInit {
    ...
    if (this.extendform.value.fileupload == "") {
      ...
      if (this.dataString.length == 0) {
        ...
        if (this.name == true && this.excellKeyValid == true) {
          ...
          const url =
            serverUrl.web + "/esim/saveEsimExtendOneYearInvoiceHeader";
          this.ajaxService.ajaxPostWithFile(url, testData).subscribe((res) => {
            if (res.message == "Extend One Year Invoice Saved Successfully") {
              ...
              res.message == "Extend One Year Invoice Not Saved Successfully"
            ) {
              this.commonService.showConfirm(res.message);
              this.button = false;
              this.clear();
  ...
  async edit() {
    if (this.extendform.value.fileupload == "") {
      ...
      const url = serverUrl.web + "/esim/saveEsimExtendOneYearInvoiceHeader";
      this.ajaxService.ajaxPostWithFile(url, testData).subscribe((res) => {
        if (res.message == "Extend One Year Invoice Saved Successfully") {
          this.commonService.showConfirm(res.message);
          this.clear();
