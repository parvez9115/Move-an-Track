...
export class ExpenseTableComponent implements OnInit {
  ...
  tableData:any;
  pdfdatas: any;
  head = ['Vehicle No', 'Trip Name', 'Cost','Expense', 'Date', 'Description'];
  ...
  createPdf() {
    this.commonService.createPdf(this.head, this.pdfdatas, "Expense Report", this.myPlatform, "Expense Report")
  }
...
var url= serverUrl.fmsUrl +'/incomeexpense/getFmsExpenseAll?companyid='+localStorage.getItem('corpId');
this.ajaxService.ajaxGet(url).subscribe(res => {
  ...
  for (var i = 0; i < detail.length; i++) {
    ...
    this.columns = [
      ...
      { text: 'Expense', datafield: 'incomeandexpense', cellsrenderer: this.renderer , cellsalign: 'center', align: 'center'},
    ...
    if (this.selectedRow) {
      const alert = await this.alertController.create({
        ...
        },
        {
          text: 'Ok',
          handler: data =>{
            const url = serverUrl.web + '/incomeexpense/deleteExpense?companyid='+localStorage.getItem('corpId')+'&id='+this.selectedRow.id+'&deletedby='+localStorage.getItem('userName');
