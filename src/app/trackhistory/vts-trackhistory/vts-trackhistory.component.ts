...
export class VtsTrackhistoryComponent implements OnInit {
  ...
  kml() {
    let _url =
      serverUrl.web +
      "/trackHistory/generateKMLServlet?name=BackTracking&selectedVehicle=" +
      this.selectedVin.vin +
      "&fromDate=" +
      this.datePipe.transform(
        this.user.fromDate,
        "yyyy-MM-dd " + this.datePipe.transform(this.user.fromDate, "h:mm:ss")
      ) +
      "&toDate=" +
      this.datePipe.transform(
        this.user.toDate,
        "yyyy-MM-dd " + this.datePipe.transform(this.user.toDate, "h:mm:ss")
      ) +
      "&compName=" +
      localStorage.corpId +
      "&brnchName=" +
    ...
    setTimeout(() => {
      if (this.gridLiveChange == undefined || this.map == undefined) {
        this.map = this.mapService.loadMap(
          ...
          { lat: this.traveVinData.latitude, lng: this.traveVinData.longitude },
          false,
          false
        );
    ...
    this.ajaxService.ajaxPostMethod(url, body).subscribe((res) => {
      if (res.length > 0) {
        ...
        this.commonService.presentAlert(
          "Record not found",
          "There is no record found in this vehicle"
        );
      }
    });
  }
  myGridOnRowSelect($event) {
    //console.log($event)
    if (!this.commonService.isLoading) this.commonService.presentLoader();
    if ($event.args.row == undefined) {
      this.userSelected.fromDate =
        this.datePipe.transform(this.user.fromDate, "yyyy-MM-dd") +
        " " +
        this.datePipe.transform(this.user.fromDate, "HH:mm:ss");
      this.userSelected.toDate =
        this.datePipe.transform(this.user.toDate, "yyyy-MM-dd") +
        " " +
        this.datePipe.transform(this.user.toDate, "HH:mm:ss");
    } else {
      this.userSelected.fromDate =
        this.datePipe.transform($event.args.row.From, "yyyy-MM-dd") +
        " " +
        this.datePipe.transform($event.args.row.From, "HH:mm:ss");
      this.userSelected.toDate =
        this.datePipe.transform($event.args.row.To, "yyyy-MM-dd") +
        " " +
        this.datePipe.transform($event.args.row.To, "HH:mm:ss");
    }
    this.getHistory();
  }
  ...
  getHistory() {
    ...
    if (this.myPlatform == "desktop") {
      url =
        serverUrl.web +
        "/device/trackhistory?vehicleNo=" +
        this.selectedVin.vin +
        "&fromDate=" +
        this.datePipe.transform(this.userSelected.fromDate, "yyyy-MM-dd") +
        "&fromtime=" +
        this.datePipe.transform(this.userSelected.fromDate, "HH:mm:ss") +
        "&totime=" +
        this.datePipe.transform(this.userSelected.toDate, "HH:mm:ss");
    } else {
      url =
        serverUrl.web +
        "/device/trackhistory?vehicleNo=" +
    ...
    this.ajaxService.ajaxGet(url).subscribe((res) => {
      ...
      if (localStorage.map != "GoogleMap") {
        this.map.on("singleclick", async (evt) => {
          ...
          feature = await this.map.forEachFeatureAtPixel(
            evt.pixel,
            (feature) => {
          ...
          if (
            feature.values_.title == "lineString" &&
            feature.values_.title !== ""
          ) {
            const url =
              serverUrl.web +
              "/login/company/latlngtoaddress/" +
              feature.values_.name.latitude +
              "/" +
              feature.values_.name.longitude +
              "/" +
              localStorage.corpId;
            this.ajaxService.ajaxGetObject(url).subscribe((res) => {
              if (this.myPlatform == "desktop") {
                if (res.length > 5) {
                  ...
                  content.innerHTML =
                    '<h2 style="margin-bottom: -26px;border: 1px solid black;margin-top: -1px;background-color: lightgrey;text-transform: uppercase;text-align: center;">' +
                    feature.values_.name.vehicleStatus +
                    "</h2><br><p>TIME:" +
                    feature.values_.name.timeStamp +
                    '</p><p style="margin-top: -11px;">SPEED:' +
                    feature.values_.name.speed +
                    '</p><p style="margin-top: -11px;">LATLNG:' +
                    feature.values_.name.latitude +
                    "," +
                    feature.values_.name.longitude +
                    "</p>" +
                    '</p><p style="margin-top: -11px;">LOCATION:' +
                    location +
                    "</p>";
                ...
                this.traveVinData["status"] =
                  feature.values_.name.vehicleStatus;
                this.traveVinData["latitude"] = parseFloat(
                  feature.values_.name.latitude
                );
                this.traveVinData["longitude"] = parseFloat(
                  feature.values_.name.longitude
                );
  ...
  trackHistoryData(data) {
    ...
    this.trackHistoryJson.forEach(function (value) {
  ...
  plotChangeMarker() {
    ...
    for (let cnt = 1; cnt < this.trackHistoryJson.length - 1; cnt++) {
      ...
      const diffMin = this.commonService.secondstoHHMMSSCal(
        new Date(json[cnt + 1]["timeStamp"]),
        new Date(json[cnt]["timeStamp"])
      );
      if (
        json[cnt]["vehicleStatus"] == "Stop" &&
        this.showdataMarker.stop == true
      ) {
        ...
        this.mapService.addMarkerWithInfoWindow(
          this.map,
          source,
          json[cnt],
          icon
        ...
        json[cnt]["vehicleStatus"] == "Idle" &&
        this.showdataMarker.idle == true
      ) {
        ...
        json[cnt]["vehicleStatus"] == "Overspeed" &&
        this.showdataMarker.overSpeed == true
      ) {
        ...
        json[cnt]["vehicleStatus"] == "Running" &&
        this.showdataMarker.running == true
      ) {
        icon = "assets/trackHistoryIcons/oldgreen.png";
        const statusForHistory = this.commonService.liveTrackingStatus(
          this.selectedVin.vin,
          "Running"
        );
        // this.createMarkerforStopIdle(source,
  ...
  async intervalLoop() {
    while (this.liveCount < this.trackHistoryJson.length) {
      var latitude = parseFloat(
        this.trackHistoryJson[this.liveCount]["latitude"]
      );
      var longitude = parseFloat(
        this.trackHistoryJson[this.liveCount]["longitude"]
      );
      if (this.trackHistoryJson[this.liveCount]["vehicleStatus"] != "") {
        this.traveVinData["status"] =
          this.trackHistoryJson[this.liveCount]["vehicleStatus"];
      ...
      this.traveVinData["timeStamp"] =
        this.trackHistoryJson[this.liveCount].timeStamp.split(" ")[0] +
        " " +
        this.trackHistoryJson[this.liveCount].timeStamp.split(" ")[1];
      this.traveVinData["speed"] =
        this.trackHistoryJson[this.liveCount]["speed"];
      // this.traveVinData['odometer'] = this.trackHistoryJson[this.liveCount]['odometer'];
      this.traveVinData["odometer"] = "";

      this.playbackUpdater["timeStamp"] =
        this.trackHistoryJson[this.liveCount].timeStamp.split(" ")[0] +
        " " +
        this.trackHistoryJson[this.liveCount].timeStamp.split(" ")[1];
      this.playbackUpdater["speed"] =
        this.trackHistoryJson[this.liveCount]["speed"];
      if (this.playBackOdometer == 0) {
        this.playBackOdometer =
          this.trackHistoryJson[this.liveCount]["odometer"] / 1000;
      } else {
        this.playBackOdometer =
          this.playBackOdometer +
          this.trackHistoryJson[this.liveCount]["odometer"] / 1000;
      ...
      if (this.liveCount > 0) {
        let prevLatLng = {
          lat: parseFloat(
            this.trackHistoryJson[this.liveCount - 1]["latitude"]
          ),
          lng: parseFloat(
            this.trackHistoryJson[this.liveCount - 1]["longitude"]
          ),
        };
        let currLatLng = {
          lat: latitude,
  ...
  playbackSpeedTrigger(speed) {
    ...
    if (this.display == "pause") this.moveVehicleInMap("play", "trigger");
  }
