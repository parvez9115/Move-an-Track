...
@Component({
  ...
  animations: [
    trigger("inOutAnimation", [
      transition(":enter", [style({ opacity: 0.8 }), animate("0.7s ease-out")]),
    ]),
    trigger("fade", [
      transition("void => *", [
        style({ opacity: 0.3 }),
...
export class GridviewTabPage implements OnInit {
  ...
  ionViewWillEnter() {
    ...
    this.ajaxService.ajaxGet(url).subscribe((res) => {
      ...
      if (
        !JSON.parse(localStorage.getItem("SubscriptionDate")) &&
  ...
  ngOnInit() {
    ...
    this.loginData = Object.keys(
      JSON.parse(localStorage.getItem("loginData"))[0]
    );
