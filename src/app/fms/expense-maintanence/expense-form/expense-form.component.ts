...
export class ExpenseFormComponent implements OnInit {
  ...
  getExpenseType() {
    var url =
      serverUrl.fmsUrl +
      "/incomeexpensegroup/getFmsExpenseGroup?companyid=" +
  ...
  gettriplist = (event: {
    ...
    if (event.value) this.expensemaintenanceForm.value.plateNo = event.value;
    var url =
      serverUrl.fmsUrl +
      "/trip/getFmsTrip?companyid=" +
      localStorage.getItem("corpId") +
      "&vin=" +
      this.expensemaintenanceForm.value.plateNo;
    this.ajaxService.ajaxGetPerference(url).subscribe((res) => {
      this.tripname = res;
    });
  };
  ...
  getcurrencydetails() {
    var url =
      serverUrl.fmsUrl +
      "/currency/getFmsCurrencySettings?companyid=" +
      localStorage.getItem("corpId");
    this.ajaxService.ajaxGetPerference(url).subscribe((res) => {
  ...
  submit() {
    ...
    if (this.currencyvalue == "" || this.currencyvalue == undefined) {
      ...
      data = {
        ...
        transactiondt:
          this.expensemaintenanceForm.value.fromDate +
          " " +
          this.expensemaintenanceForm.value.fromtime +
          ":00",
      ...
      (data["vin"] = this.expensemaintenanceForm.value.plateNo),
        ...
        (data["incomeandexpense"] =
          this.expensemaintenanceForm.value.incomeExpense),
      ...
      this.ajaxService.ajaxPostWithBody(url, data).subscribe((res) => {
        if (res.message == "Income & Expense Update Successfully") {
          this.commonService.presentToast(
            "Expense Details Updated Succesfully"
          );
          this.modalController.dismiss();
        } else {
          this.commonService.presentToast("Please Contact Support Team");
