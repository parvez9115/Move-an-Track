...
export class VtsGeofenceComponent implements OnInit {
  ...
  getGeoFenceData(data) {
    ...
    const url =
      ...
      this.zoneType +
      "&vechicle=" +
      JSON.parse(localStorage.getItem("selectedVin"))["vin"];
    this.ajaxService.ajaxGet(url).subscribe(
      (res) => {
        console.log(res);
      ...
      const alert = await this.alertController.create({
        ...
        buttons: [
          ...
          },
          {
            ...
            handler: (data) => {
              const url =
                serverUrl.web + "/zone/deletezone?id=" + selectedJsonFormat.id;
              this.ajaxService.ajaxDeleteWithString(url).subscribe(
                (res) => {
                  console.log(res);
                  this.ionViewWillEnter();
                  this.commonService.presentToast(
                    "Your zone deleted successfully"
                  );
                  this.commonService.dismissLoader();
                  this.selectedZoneData = [];
                },
