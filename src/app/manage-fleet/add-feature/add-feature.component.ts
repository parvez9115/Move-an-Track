...
export class AddFeatureComponent implements OnInit {
  ...
  submitData() {
    ...
    var data = {
      compId: this.value.companyId,
      userId: this.value.userName,
      oldCategoryRole: "CompanyAdmin",
    };
    const url =
      serverUrl.web +
      "/user/update/userfeatures?compId=" +
      this.value.companyId +
      "&userId=" +
      this.value.userName;
    this.ajaxService.ajaxPostWithString(url, postDatas).subscribe((res) => {
      var resData = JSON.parse(res);
      resData.message == "Persisted"
