...
export class StudentLoginComponent implements OnInit {
  ...
  checkUser() {
    if (
      this.login.value.mobileNo != null &&
  ...
  generateOTP = (cntrl) => {
    ...
    if (/(android|iPhone|iPad|iPod)/i.test(navigator.userAgent)) {
      if (
        this.login.value.mobileNo === "9600696008" ||
  ...
  sideMenus() {
    ...
    const url =
      serverUrl.web + "/parentapp/login?parentId=" + this.login.value.mobileNo;
    this.ajaxService.ajaxGet(url).subscribe((res) => {
      console.log(res);
          ...
          this.firebaseX
            ...
            .catch((error) => {
              ...
              this.commonService.presentToast(
                "Something went wrong!!Push notification won't recieve"
              );
            });
