  ...
  submit() {
    ...
    if (this.serviceName != "available") {
      const data = {
        ...
        useraddress1: this.userForm.value.Address1,
        useraddress2: this.userForm.value.Address2,
        userExpiryDate: date,
        userCity: this.userForm.value.city,
        contactNo: this.userForm.value.contact,
      ...
      this.ajaxService.ajaxPostWithString(url, data).subscribe((res) => {
        ...
        if (res == "persisted") {
          ...
          this.commonService.showConfirm("User already exists");
        }
      });
    } else {
      var details = {
