...
export class ReceivedDetailsComponent implements OnInit {
  ...
  submitBtn() {
    ...
    const url = serverUrl.web + "/esim/saveEsimCertificateInvoiceDetails";
    this.ajaxService
      ...
      .subscribe((res) => {
        if (
          ...
          res.message == "Certificate Invoice Detail Not Saved Successfully"
        ) {
          this.commonService.showConfirm(
            "Certificate Invoice Detail Not Saved Successfully"
          );
        } else {
          this.commonService.showConfirm(
