...
let ManufactureDetailPageModule = class ManufactureDetailPageModule {
};
...
let ManufactureDetailPage = class ManufactureDetailPage {
    constructor(ajaxService, router, formBuilder, commonService, modalController) {
        ...
        this.today = new Date();
        this.getpurchaseorderlist = (event) => {
            if (event.value)
                this.ManufactureForm.value.PurchaseOrderNo = event.value;
        };
        this.getmfgunit = (event) => {
            if (event.value)
                this.ManufactureForm.value.mfgunit = event.value;
        };
    }
    onFileChange(ev) {
...
ManufactureDetailPage.ctorParameters = () => [
...
//# sourceMappingURL=manufacture-detail-manufacture-detail-module-es2015.js.map