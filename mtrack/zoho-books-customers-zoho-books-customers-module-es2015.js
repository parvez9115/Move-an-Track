...
let ZohoBooksCustomersPageModule = class ZohoBooksCustomersPageModule {
};
...
let ZohoBooksCustomersPage = class ZohoBooksCustomersPage {
    ...
    getcustomer() {
        const url = src_environments_environment__WEBPACK_IMPORTED_MODULE_8__["serverUrl"].web + "/esim/getZohoBooksCustomers";
        this.ajaxService.ajaxGet(url).subscribe((res) => {
            this.books = res;
        });
    }
    ...
    setValue() {
        ...
        let data = this.books.filter((f) => f.customername == this.selectedRow.customername);
        this.product = data[0];
    }
    selectvalue(ev) {
...
ZohoBooksCustomersPage.ctorParameters = () => [
...
//# sourceMappingURL=zoho-books-customers-zoho-books-customers-module-es2015.js.map