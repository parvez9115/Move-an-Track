...
export class FleetFormComponent implements OnInit {
        ...
        .subscribe((res) => {
          ...
          if (data == null) {
            this.commonService.presentToast(
              "your request wasn't solved, Kindly Contact support team"
            );
  ...
  fleetSubmition() {
    if (this.fleetLogin.value.vehicleType == "REFRIGERATOR") {
      if (
        this.fleetLogin.value.assertLocation == null ||
        this.fleetLogin.value.assertAddress == null ||
        this.fleetLogin.value.assertLocation == undefined ||
        this.fleetLogin.value.assertAddress == undefined ||
        this.fleetLogin.value.assertLocation == "" ||
        this.fleetLogin.value.assertAddress == ""
      ) {
        this.commonService.presentToast(
          "Please enter assert location and address"
        );
        return null;
      }
    }
  ...
  getOperators() {
    ...
    var datas = {
      compnyID: companyDetail.companyID,
      branchID: companyDetail.branchID,
      companyNme: companyDetail.userId + "",
    };
    var url2 =
      serverUrl.web +
      "/operator/operatorinfo?companyId=" +
      companyDetail.companyID +
      "&branchId=" +
      companyDetail.branchID;
  ...
  vehicleDetails() {
    ...
    if (this.fleetLogin.value.hours) {
      engineHours =
        this.fleetLogin.value.hours +
        ":" +
        this.fleetLogin.value.minutes +
        ":" +
        this.fleetLogin.value.seconds;
      totalEngineHours =
        this.fleetLogin.value.days +
        ":" +
        this.fleetLogin.value.totalhours +
        ":" +
        this.fleetLogin.value.totalMinutes +
        ":" +
        this.fleetLogin.value.totalSeconds;
    }
    ...
    unCheckedValues = this.fleet.filter(
      (val) => !this.fleetLogin.value.fleetManager.includes(val)
    );
    ...
    var addDetails = {
      ...
      imeiNo: this.fleetLogin.value.imei.toString(),
      userEntry: "kingstrackalerts@gmail.com",
      groupColor: "",
      fleetUser:
        fleetManage + "," + this.fleetLogin.value.fleetManager.toString(),
      plateNo: this.fleetLogin.value.plateNo,
      oldCheckRoad: 0,
      oldcheckFreeForm: 0,
      oldcheckGeoZone: 0,
      oldchecklandMark: 0,
      oldcheckshift: 0,
      type: "BikeApp",
      icon: this.fleetLogin.value.vehicleType,
      iconUrl: this.fleetLogin.value.vehicleType,
      model: "Model",
      group: "true",
      dateofpurchase: "true",
      ...
      regexpiry: "true",
      mileageInit:
        this.fleetLogin.value.km +
        "-" +
        this.fleetLogin.value.fuelType +
        "-" +
        this.fleetLogin.value.cost +
        "",
      odometer: this.fleetLogin.value.travelledKm.toString(),
      preventiveselect: 3000,
      preventivelimitselect: 30,
      scheduleselect: 3000,
      schedulelimitselect: 30,
      PreventiveMaintenanceType: "hours",
      operatorID: this.fleetLogin.value.operator,
    ...
    if (this.serviceName != "available") {
      this.ajaxService.ajaxPostWithString(url, addDetails).subscribe((res) => {
        ...
        if (res === "persisted") {
              ...
              if (JSON.parse(res).message == "Success") {
                ...
                if (JSON.parse(res).geozone == "Success") {
                  const detailsForAssign = {
                    vin: this.vin,
                    geoIds: [JSON.parse(res).geoId.toString()],
                    operation: "association",
                  };
                  const url = serverUrl.web + "/zone/geozone/vehicle/associate";
                  this.ajaxService
                    .ajaxPostWithString(url, detailsForAssign)
                    .subscribe(
                      (res) => {
                        //console.log(res);
                        this.dashboardData();
                        ...
                        let body = {
                          vin: this.vin,
                          method: "alertconfigCache",
                        };
                        this.ajaxService
                          .ajaxPostWithString(url2, body)
                          .subscribe((res) => {
                            if (res == "Done") {
                              this.commonService.presentToast(
                                "Alerts are enabled"
          ...
          this.commonService.presentToast(
            " Vehicle not added, Please check the details "
          );
          this.displayBar = false;
        }
      });
      ...
      var data = {
        ...
        plateNo: this.fleetLogin.value.plateNo,
        odometer: JSON.stringify(this.fleetLogin.value.travelledKm),
        mileageInit:
          this.fleetLogin.value.km +
          "-" +
          this.fleetLogin.value.fuelType +
        ...
        iconUrl: this.fleetLogin.value.vehicleType,
        operatorID: this.fleetLogin.value.operator,
        checkedUser: this.fleetLogin.value.fleetManager.toString(),
        uncheckedUser: unCheckedValues.toString(),
        location:
          this.fleetLogin.value.assertLocation +
          "|" +
          this.fleetLogin.value.assertAddress,
      ...
      var count = 0;
      this.ajaxService
        .ajaxPostWithErrorBody(url1, JSON.stringify(data))
        .subscribe((res) => {
          //   //console.log(res);
          if (count == 0) {
            count = 1;
  ...
  dashboardData() {
    ...
    this.websocketService.reSendRequest(
      JSON.parse(localStorage.dashboardWebSocketData)
    );
    // let data = {
  ...
  generateVin() {
    if (this.fleetLogin.value.imei != "" || this.fleetLogin.value.imei != " ") {
      const url =
        serverUrl.web +
        "/login/getPreferences?key=vinGenerationValue&companyId=" +
        this.companyDetail.companyID;
      this.ajaxService.ajaxGetPerference(url).subscribe((res) => {
        const vehicleVin =
          this.selectedCompany + (Math.floor(Math.random() * res) + 100);
        ...
        buttons: [
          ...
          },
          {
            ...
            handler: (data) => {
              ...
              this.ajaxService
                .ajaxDeleteWithBody(url, deleteData)
                .subscribe((res) => {
                  //console.log(res);
                  if (res.error.text == "Operator is already Associated") {
                    this.commonService.presentToast(
                      "Associated operator cannot be deleted"
                    );
                  } else if (
                    res.error.text == "persisted" ||
                    res.statusText == "OK"
                  ) {
  ...
  currentLocation(mode) {
    this.geolocation
      ...
      .then((resp) => {
        //console.log(resp)
        this.fleetLogin.patchValue({
          assertLocation: resp.coords.latitude + "," + resp.coords.longitude,
        });
      })
      .catch((error) => {
  ...
  mapReturnFunction(mode) {
    this.fleetLogin.patchValue({
      assertLocation: localStorage.getItem("mapLocationPicker"),
    });
    ...
    if (this.value) {
      if (this.value.submit == "available") {
        ...
        this.fleetLogin.patchValue({
          ...
          km: mileageInit[0],
          operator: this.value.operator == "" ? "0" : this.value.operator,
          hours: todayArr[0][0],
